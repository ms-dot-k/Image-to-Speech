...
class UnitDataset(Dataset):
    def __init__(self, image_path, speech_unit_path, split_path, mode, num_sp_unit=200, max_sp_len=1024, test_data='coco'):
    ...
    def build_file_list(self, image_path, speech_unit_path, split_path, mode):
        ...
        if mode == 'train':
            ...
            for d in data['images']:
                if d['split'] == mode:
                    im_path = os.path.join(fl_im, d['filename'])
                    sp_path = os.path.join(fl_sp, 'flickr_audio/wavs', d['filename'][:-4] + f'_0.unit')
                    im_files.append(im_path)
                    sp_files.append(sp_path)
    ...
    def __getitem__(self, idx):
        ...
        im_input = transform(image).unsqueeze(0)
