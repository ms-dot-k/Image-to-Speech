...
class TokenVisionTransformer_GIT(VisionTransformer):
    def __init__(self, global_pool=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def forward(
        ...
        cls_token = self.cls_token.expand(pixel_values.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_token, pixel_values), dim=1)
        x = self.pos_drop(x + self.pos_embed)
        x = self.blocks(x)
...
def deit_base_token_32(pretrained=False, **kwargs):
    model = TokenVisionTransformer_GIT(
